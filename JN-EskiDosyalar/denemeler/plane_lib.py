...
class Plane():
    ...
    def smplgo(self,konum):

        done = False
        self.vehicle.simple_goto(konum)
        print("going..")
        ...
        while True:
            ...
            current_x = self.vehicle.location.global_relative_frame.lat
            current_y = self.vehicle.location.global_relative_frame.lon
            #print("current_x:", current_x, "current_y:", current_y, "x:", x, "y:", y)
            #time.sleep(0.5)
            if (x - 0.0005 < current_x < x + 0.0005 and y - 0.0005 < current_y < y + 0.0005):
                done = True
                break
    ...
    def change_heading(self,deg,alt):
        ...
        while True:
            print("att_heading", self.att_heading_deg, "target_deg", deg)
            self.set_ground_course(deg, alt)
            #self.set_ground_course(self.att_heading_deg+10, alt)
            if (deg - 5 < self.att_heading_deg < deg + 5):
                break
    ...
    def txt_save(self,i,deg,cx,cy):

        dosya2 = open("C:/Users/ACP/Desktop/data/data.txt", 'a+')
        satir1 = str(i)
        satir2 = str(deg)
        satir3 = str(cx)
        satir4 = str(cy)
        dosya2.write("\n" + satir1 + "," + satir2 + "," + satir3 + "," + satir4)
        dosya2.close()
...
class Image():
    ...
    def image_save(self,count,i):
        ...
        lower_red = np.array([0, 142, 72])  # example value
        upper_red = np.array([9, 255, 255])  # example value
        ...
        cv2.imwrite("C:/Users/ACP/Desktop/data/" + str(i) + ".jpg", hsv_mask)
        cv2.imwrite("C:/Users/ACP/Desktop/data/" + str(i) + "orj" + ".jpg", frame)
